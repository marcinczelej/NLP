...
class TransformerLayersTest(unittest.TestCase):
    ...
    def test_positionalEncoding_shape(self):
        ...
        input_data = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))
    ...
    def test_MultiHeadAttentionLayer_shape(self):
        ...
        q = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))
        k = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))
        v = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))

        x = tf.random.uniform(shape=(16, self.actual_seq_length))
        seq_mask = makeSequenceMask(x.shape[1])

        desired_shape = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))
    ...
    def test_EncoderLayer_shape(self):
        ...
        input = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))

        x = tf.random.uniform(shape=(16, self.actual_seq_length))
        pad_mask = makePaddingMask(x)


        assert(encoder_layer(input, pad_mask, training_enabled=True).shape == input.shape)
    ...
    def test_DecoderLayer_shape(self):
        ...
        x = tf.random.uniform(shape=(16, self.actual_seq_length))
        encoder_output = tf.random.uniform(shape=(16, self.actual_seq_length, self.embedding_size))
        decoder_input = tf.random.uniform(shape=(16, self.decoder_actual_seq_len, self.embedding_size))

        seq_mask = makeSequenceMask(decoder_input.shape[1])
        pad_mask = makePaddingMask(x)

        assert(decoder_layer.call(decoder_input, encoder_output, pad_mask, seq_mask, training_enabled=True).shape == decoder_input.shape)